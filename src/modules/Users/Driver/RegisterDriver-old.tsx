...
const RegisterDriver: React.FC<RegisterDriverProps> = ({
  onClose,
  setHasSubmitted,
}) => {
  ...
  useEffect(() => {
    const fetchStates = async () => {
      try {
        const response = await lookupService.getStates()
        setStates(response)
      } catch (error) {
        toast({
          title: "Error",
  ...
  const handleStateChange = async (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    setFormValues({ ...formValues, licenseState: stateName, lga: "" })

    if (selectedState?._id) {
      try {
        const lgaList = await lookupService.getLGAs(selectedState._id)
        const sorted = lgaList.map((lga: any) => lga.name).sort()
        setLgas(sorted)
      } catch (error) {
        toast({
          title: "Error",
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        vehicleImages: vehiclePicturePaths.map((pic) => pic.path),
      ...
      setHasSubmitted(true) // 🔁 Triggers dashboard update
      onClose()
    } catch (error) {
