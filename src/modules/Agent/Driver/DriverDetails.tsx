...
import DriverInfo from "@/reusables/DriverInfo"
...
const DriverDetails: React.FC = () => {
  ...
  const mockDetails = {
    ...
    OverdueCharges: "5000", // Added missing Driver
  }
  ...
  const agentTotal = [
    {
      ...
      value: `₦${new Intl.NumberFormat("en-GB").format(
        DriverDetails?.totalTransactionValue?.currentMonth ?? 0
      )}`,
      percentage: getPercentageChange(
        parseInt(DriverDetails?.registeredUsers?.currentMonth),
        parseInt(DriverDetails?.registeredUsers?.previousMonth)
      )?.percentageChange,
    },
    {
      id: 2,
      icon: totalTransactions,
      text: `Driver value as of ${new Date().toLocaleString("en-US", {
        month: "long",
      })}`,
      value: `₦${new Intl.NumberFormat("en-GB").format(
        DriverDetails?.totalTransactionValue?.currentMonth ?? 0
      ...
      percentage: getPercentageChange(
        parseInt(DriverDetails?.totalTransactionValue?.currentMonth),
        parseInt(DriverDetails?.totalTransactionValue?.previousMonth)
      )?.percentageChange,
    },
  ]
