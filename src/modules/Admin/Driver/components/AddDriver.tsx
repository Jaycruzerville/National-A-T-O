...
const AddDriver: React.FC = () => {
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      ...
      DriverOwnerName: Yup.string().required("Required"),
      assignedDriverId: Yup.string().required("Required"),
    }),
    onSubmit: async (values) => {
      try {
        ...
        const payload = {
          ...
          DriverOwnerName: values.DriverOwnerName,
    ...
    >
      <form onSubmit={formik.handleSubmit}>
        <Flex flexDir="column" gap="12px">
          ...
          <FormControl isInvalid={!!formik.errors.DriverOwnerName}>
            <AppFormLabel title="Driver Owner Name" />
            <Input
              placeholder="Enter Driver Owner Name"
              name="DriverOwnerName"
              onChange={formik.handleChange}
              value={formik.values.DriverOwnerName}
            />
            {!!formik.errors.DriverOwnerName && (
              <Text color="red" fontSize="sm">
                {formik.errors.DriverOwnerName}
              </Text>
            )}
          ...
          <FormControl isInvalid={!!formik.errors.assignedDriverId}>
            <AppFormLabel title="Assigned Driver ID" />
            <Input
              placeholder="Enter Assigned Driver ID"
              name="assignedDriverId"
              onChange={formik.handleChange}
              value={formik.values.assignedDriverId}
            />
            {!!formik.errors.assignedDriverId && (
              <Text color="red" fontSize="sm">
                {formik.errors.assignedDriverId}
              </Text>
            )}
